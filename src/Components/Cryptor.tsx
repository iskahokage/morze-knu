...
const Cryptor: React.FC<cryptorProps> = ({ trigger }) => {
    type EventHandler = (e: React.ChangeEvent<HTMLInputElement>) => string;
    ...
    const encodeToMorse = () => {
        // const replacedSpace = text.replace(/ /g, "|");
        ...
        trigger();
        localStorage.setItem("crypted", JSON.stringify(cryptedResult));
    };

    return (
        <div className="container mx-auto bg-gray-200 rounded-lg p-5 grid gap-4">
            <h2 className="text-lg">Напишите текст для шифровки в морзе:</h2>
            <textarea
